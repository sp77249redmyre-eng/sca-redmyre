...
### works.html — comment JSONB 구조 확인
- `internal_note` 컬럼 없음 — 잘못된 정보 수정
- 올바른 구조: `comment` JSONB 필드의 `note` 키 사용
- 가격 정보는 반드시 `comment.note` (Internal Notes) 에만 — Scope/Notes 필드 절대 금지

### BMS 가격 노출 정책 (확정)
- Scope/Notes 필드 = 공개 — 가격 절대 기재 금지
- Internal Notes (comment.note) = 내부용 — 가격 여기에만

### Gmail MCP 패턴 (확정)
- `Gmail:create_draft` 파라미터: `to`, `cc`, `bcc`, `subject`, `htmlBody`, `replyToMessageId`
- HTML 이메일: `border="1" cellpadding="8" cellspacing="0"` + inline style (CSS class 금지)
- width="100%" 사용 (fixed pixel width 금지 — Gmail 렌더링 깨짐)
- BCC 기본: sca.yun82@gmail.com, sca.jacob77@gmail.com (Jacob 지시 시에만)
- sp77249.redmyre@gmail.com 만 MCP 연결 — sca.jacob77@gmail.com 아웃박스 접근 불가

### Edge Function 이메일 정책 (2026-04-21 확정)
- complaints 이메일 완전 제거 (신규 제출/관리자 답변/입주자 답변 모두)
- Push 알림만 유지
- quotes 이메일 유지 (위원회 의사결정 신뢰성)
- parking 이메일 유지 (익명 차량 소유자 BMS 계정 없음)
