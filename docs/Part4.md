...
# 📋 2026-05-17 작업 기록 — UI 개선 (Hero 제거 + 사이드바 애니메이션)

## 작업 1: sidebar.html — inline SVG + hover 애니메이션

Tabler 웹폰트 `<i class="ti ti-xxx">` → inline SVG 전환. 19개 메뉴 아이콘 각 특성별 CSS hover 애니메이션 적용.

| 메뉴 | 애니메이션 |
|---|---|
| Overview | 지붕 bounce |
| Service Reports | 체크선 draw |
| SCM & Documents | 위 파일 들림 |
| Announcements | 메가폰 좌우 흔들림 |
| Parking | 차 전체 오른쪽으로 달려나가고 왼쪽 재등장 루프 |
| Occupant Details | 빌딩 rise |
| Resident Requests | 봉투 뚜껑 열림 |
| A/C Request | 바람선 파동 |
| A/C History | 시계바늘 회전 |
| Quote Approvals | 스캔선 내려옴 |
| Active Works | 렌치 회전 |
| Completed Works | 체크 bounce |
| Cost Report | 막대 rise |
| Emergency | 폰 진동 |
| User/Committee Guide | 페이지 넘김 |
| User Management | 두 번째 사람 슬라이드 |
| System Management | 기어 90도 회전 |
| Signboard | TV 화면 깜빡 |

layout.js/topbar.html 미수정. CSS animation + inline SVG 교체만.

---

## 작업 2: style.css 변경사항

**.page-header (신규 추가 — Hero 대체 공통 헤더):**
- icon: 48×48px, border-radius:12px, background:#eff6ff
- svg: 28×28px, stroke:#2563eb
- title: font-size:20px, font-weight:700, letter-spacing:-0.4px
- sub: font-size:12px, color:var(--muted)
- 모바일(768px): icon 30px, svg 15px, title 17px

**.section-title 수정:**
- font-size: 12px → 15px
- color: var(--text-2) → var(--text)
- text-transform:uppercase 제거, letter-spacing 제거

**.section-hdr 수정:**
- justify-content:space-between 제거
- gap:12px 추가 (버튼 제목 바로 옆)

---

## 작업 3: Hero 제거 → page-header 교체 (14개 파일)

그라데이션 Hero 삭제 → page-header 교체. 버튼은 title-row 안으로. 배너는 아래 독립 div 유지. 배너 border-radius `0 0 16px 16px` → `10px`.

| 파일 | 버튼 | 배너 |
|---|---|---|
| announcements | `+ New Announcement` → 제목 옆 | push-banner 유지 |
| complaints | `+ New Request` → 제목 옆, btn-primary 제거, padding:6px 14px, font-size:12px | 초록 유지 |
| cost-dashboard | 없음 | 초록 배너 삭제. Completed Works margin-left:auto 제거 |
| documents | 없음 | 없음. padding:0 제거 |
| emergency | `Call 000` → 빨간 배너 텍스트 인라인 | 빨간 유지 |
| history | `Clear All` → 필터바 Refresh 옆 | 없음 |
| hvac | 없음 | 노랑 유지 |
| occupants | `Export Excel` → 제목 옆 | 파랑 유지 |
| parking | 없음 | 빨간 유지 |
| quotes | 없음 | 파랑 투표룰 유지 |
| reports | 없음 | 회색 배너 삭제 |
| service-reports | 없음 (sr-hero → page-header 교체, srHeroTitle/srHeroSub ID 유지) | 없음 |
| users | `+ Invite User` → 제목 옆 | 없음 |
| works | `Emergency Work` → 제목 옆. worksCount display 토글 JS 추가 | 회색 삭제 |

works.html worksCount JS:
```js
// 변경 전
if (cnt) cnt.textContent = allWorks.length || '';
// 변경 후
if (cnt) { cnt.textContent = allWorks.length || ''; cnt.style.display = allWorks.length ? 'inline-flex' : 'none'; }
```
...
## 📝 업데이트 내역 (2026-04-25 이후)

**버전:** 1.1
**업데이트일:** 2026-06-13

### 진행 중 주요 안건 (2026-06 기준)

**HVAC 교체 프로젝트**
- 18개 Pumyang 수냉식 패키지 유닛 교체 (Level 1-6)
- RFQ 발송 완료, Halil이 배포
- Voyager Technical Services 견적 수령
- iControl Systems BMS 연동: tenderer가 현장 조사에서 직접 평가 + BMS 하드웨어/소프트웨어 별도 라인 아이템으로 견적

**AFSS 2026**
- DDE Services Electrical (Loren Druzinec, jobs@ddeservices.com.au)
- Q-2981 (Common Area), Q-2982 (Units) 완료
- Q-3179 Ground Suite 재검사: Halil → Fabian (Strathfield Partners) 통해 접근 조율 중
- Unit 6H 비상조명 교체 승인 완료

**TK Elevator**
- Lift No.2 K06 컨탁터 반복 결함
- Lift No.1 연속 3회 갇힘 사고 (2026-05-27~29) → 운행 중단
- MAX Plus IoT 모니터링 Quote Ref: 2032171-06.01 ($80+GST/월 + $3,000+GST 설치)
- 위원회 이메일 발송 완료, SCM 안건 상정 예정

**롤러셔터 드럼 교체**
- ADG Quote #482 ($5,680+GST, 50% 선입금)
- 위원회 알림 발송, BMS 투표 진행 중
- ADG 연락처: Abdullah (info@automateddoorsandgates.com.au)

**지붕 누수**
- Lion Builders (Robert Trad): 조사 견적 $12,496 — 보류
- TCC Roofing Services (Eugene, info@tccroofing.com.au): 견적 진행 중
- East Coast Roof Leaks: $700 제안 — 무시
- Buildline: 견적 대기 중

**긴급출동비 제안**
- June SCM 안건: 긴급출동 최소 2시간 + 30분 단위
- 시장가 참조: ERI SalaryExpert + SEEK Sydney 2026 + 25% contractor 조정
- 현재 계약에 "no additional charge" 문구 있으나 법적 구속력 없음

### 계약 목표
- 현재: $47,227+GST/년 (미서명), Jacob 주$750 현금, 수익 40%
- 2027년 재협상 목표: ~$70,000/년
- 단계: ① 긴급출동비 승인 ② 주급 인상 ③ 2027 재협상

### Integriti 접근 제어
- TP8~TP11, PA4~PA7 생성 (Level 3~6 주말 층별 제어)
- Saturday/Sunday 체크박스 + 시간 범위 조정 보류
- 참고: Integriti_Lift_Floor_Time_Control_Guide.docx

### 신규 서비스 업체
- Smarthome Electrical (Ethan, admin@smarthomeelectrical.com.au) — 주 전기 업체로 지정
- LOCKEY.com.au (Oskar Bock) — 잠금장치
