...
### GitHub 레포 변경
- `sp77249redmyre-eng/sca-redmyre` → **Public**으로 변경 (2026-06-13)
- 사유: Claude Project GitHub sync 연동
- 보안 검토 완료: anon key (공개용), Service Role Key (Vercel 환경변수만), HVAC credentials (GitHub 미포함)

### 신규 테이블
- `qr_analytics` — QR 스캔 이벤트 추적 (event_type, created_at timestamptz)
  - 이벤트: lift_entry, lift_resident, lift_visitor, signboard_entry
  - system.html에서 7/30/90일 단위 조회 가능

### Supabase 신규 정책 (2026-05-30~)
- 신규 프로젝트: 2026-05-30부터 auto-grant 중단
- 기존 프로젝트: 2026-10-30부터 적용
- 신규 테이블 생성 시 반드시 수동 GRANT 필요:
  ```sql
...
### quotes.html — draft 가시성 수정
- 이전: committee/observer가 draft 상태 견적 볼 수 있었음
- 이후: committee/observer는 draft 완전 비표시 (admin만 가능)
- Send to Committee → status='voting' 된 후부터만 committee/observer에 표시

### occupants/vehicles 백업 테이블
- `occupants_backup_20260426`, `vehicles_backup_20260426` — 1주일+ 안정 확인 후 DROP 예정
- 멀티유닛 유저(jskmhc, laurachoo2) 정상 동작 확인됨

### index.html (로그인 페이지) 개선
- 첫 화면: building_crop.png (건물 하단 크롭), 콘텐츠 위치 margin-top:-140px 조정
- 모바일 로그인 화면: building.png 풀스크린 배경, 흰 박스 제거, 텍스트 흰색+그림자
- 로고: 흰 배경 제거 (PNG 투명 처리), 모바일에서 가운데 정렬
- building.png (전체), building_crop.png (크롭) 두 파일 분리 운영
