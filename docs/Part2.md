...
### quotes.html — 주요 변경
- **draft 가시성**: committee/observer에게 draft 완전 숨김 (renderQuotes 필터 수정)
- **투표 로직 전수 검증 완료**: 165개 조합 ALL PASS, 16개 테스트 케이스 코드 내장
- **복수 견적 동률(Tie)**: hasTie=true 시 재투표 허용, UI "⚖️ Tie — Please reconsider" 표시
- **확정 시점 재계산**: confirmSelection에서 votes 테이블 재계산 (UI 값 무시) — 조작 차단

### works.html — RTE 툴바 추가
- Edit 모달 + Emergency(Direct Work Order) 모달에 Rich Text 툴바 추가
- Bold/Italic/Underline/Color 옵션 (quotes 페이지 스타일 동일)
- contenteditable div로 textarea 교체
- 모달 너비 780px, textarea min-height 140px

### system.html — QR Analytics 개선
- QR Today (당일 스캔 수) 추가
- Signboard 항목 제거
- No selection 행 추가

### parking.html — 카메라 업로드 버그 수정
- iOS/Android에서 change 이벤트 미발화 문제
- `display:none` → `position:fixed;top:-9999px` 변경
- `input` 이벤트 리스너 추가 (change와 병행)
